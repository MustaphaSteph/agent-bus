...
- **Local web cockpit** — `agent-bus ui` opens a Slack-style local dashboard: a project rail, a team/view rail, and a main pane for team chat, Kanban, activity, a People roster (online / idle / working / blocked / waiting / stale), and an Attention view of what needs a human next, plus a drill-down inspector for any agent, task, or message. Switch projects and teams live in the browser — no restart. Read-only by design.
