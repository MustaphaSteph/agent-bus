...
- **MCP server** exposing 63 tools an agent can call (`register`, `send`,
  `inbox`, `ask`, `ask_async`, `reply`, `ack`, `ask_best`, `subscribe`,
...
- **CLI** (`agent-bus`) for the human: local web cockpit, live watch,
  team chat, task view, inject, pause, register, install Stop hook,
  generate listener prompts.
